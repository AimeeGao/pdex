<?php

namespace App\Providers;

use App\Events\IndividualCreated;
use App\Events\IndividualUpdated;
use App\Listeners\LogIndividualActivity;
use App\Listeners\SendIndividualCreatedNotification;
use App\Listeners\SendVerificationStatusNotification;
use Illuminate\Foundation\Support\Providers\EventServiceProvider as ServiceProvider;

class EventServiceProvider extends ServiceProvider
{
    /**
     * The event to listener mappings for the application.
     *
     * @var array<class-string, array<int, class-string>>
     */
    protected $listen = [
        IndividualCreated::class => [
            SendIndividualCreatedNotification::class,
        ],
        
        IndividualUpdated::class => [
            LogIndividualActivity::class,
            SendVerificationStatusNotification::class,
        ],
    ];

    /**
     * Register any events for your application.
     */
    public function boot(): void
    {
        parent::boot();
    }

    /**
     * Determine if events and listeners should be automatically discovered.
     */
    public function shouldDiscoverEvents(): bool
    {
        return false;
    }
}
