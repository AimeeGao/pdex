...
export default {
    ...
    computed: {
        hasAdminRole() {
            ...
            const adminRoles = ['Ministry_Admin'];
...
<template>
    <div class="flex items-center gap-4">
        ...
        <nav class="hidden md:flex items-center gap-2 bc-nav">
            ...
            <Link 
                href="/dashboard" 
                :class="['bc-nav-link', { 'active': $page.url === '/dashboard' }]"
            >
                Dashboard
            </Link>
            ...
            <template v-if="hasAdminRole">
                <Link 
                    href="/admin" 
                    :class="['bc-nav-link', { 'active': $page.url.includes('/admin') }]"
                >
                    Admin
                </Link>
                <Link 
                    href="/admin/users" 
                    :class="['bc-nav-link', { 'active': $page.url.includes('/admin/users') }]"
                >
                    Users
                </Link>
                <Link 
                    href="/admin/intake" 
                    :class="['bc-nav-link', { 'active': $page.url.includes('/admin/intake') }]"
                >
                    Intake
                </Link>
            </template>
            ...
            <template v-if="hasInstitutionRole">
                <Link 
                    href="/institution" 
                    :class="['bc-nav-link', { 'active': $page.url.includes('/institution') }]"
                >
                    Institution
                </Link>
                <Link 
            ...
            <!-- Student Links (only for student users) -->
            <template v-if="hasStudentRole">
                <Link 
                    href="/student" 
                    :class="['bc-nav-link', { 'active': $page.url.includes('/student') }]"
                >
                    Student
                </Link>
                <Link 
                    href="/student/profile" 
                    :class="['bc-nav-link', { 'active': $page.url.includes('/student/profile') }]"
                >
                    Profile
                </Link>
            </template>
            ...
            <template v-if="hasMinistryRole">
                <Link 
                    href="/ministry" 
                    :class="['bc-nav-link', { 'active': $page.url.includes('/ministry') }]"
                >
                    Ministry
                </Link>
                <Link 
        ...
        <div v-if="showingNavigationDropdown" class="absolute top-full left-0 right-0 bc-mobile-nav md:hidden">
            <!-- Dashboard Link -->
            <Link href="/dashboard" class="bc-nav-link">Dashboard</Link>
            
            <!-- Admin Links (only for admin users) -->
            <template v-if="hasAdminRole">
                <Link href="/admin" class="bc-nav-link">Admin</Link>
            ...
            <!-- Institution Links (only for institution users) -->
            <template v-if="hasInstitutionRole">
                <Link href="/institution" class="bc-nav-link">Institution</Link>
                <Link href="/institution/applications" class="bc-nav-link">Applications</Link>
            </template>
            
            <!-- Student Links (only for student users) -->
            <template v-if="hasStudentRole">
                <Link href="/student" class="bc-nav-link">Student</Link>
                <Link href="/student/profile" class="bc-nav-link">Profile</Link>
            </template>
            
            <!-- Ministry Links (only for ministry users) -->
            <template v-if="hasMinistryRole">
                <Link href="/ministry" class="bc-nav-link">Ministry</Link>
                <Link href="/ministry/reports" class="bc-nav-link">Reports</Link>
            </template>
